/*
    ...
    Version history:
    2024-03-13        v1.1.0        Analog output scaling corrections. Moved to PlatfomIO
...
float AFR = 0;                     /* Air-fuel ratio */

// Constants
const float STOICH_PETROL = 14.7; /* Petrol stoichiometric AFR */
const float LOW_AFR = 12.1;
const float HIGH_AFR = 18.1;
...
void UpdateAnalogOutput()
{
  ...
  // Calculate ADC value using lambda value. Range for 12.13 to 18.04 AFR
  if (AFR >= LOW_AFR && AFR <= HIGH_AFR)
  {
    analogOutput = mapfloat(AFR, LOW_AFR, HIGH_AFR, 0.0f, 255.0f);
  }
  ...
  if (!faulted)
  {
    ...
    {
    case 0:
      LEDdisplay.showNumberDec(AFR * 10, 32, false, 4, 0);
      break;
    case 1:
      ...
      LEDdisplay.showNumberDec(LAMBDA_VALUE * 100, 64, leadingZeros, 4, 0);
      break;
    }
  }
...
void setup()
{
  ...
  LEDdisplay.showAnimation(PWR, FRAMES(PWR), TIME_MS(2750));
...
void start()
{
  ...
  while (t < 5 && analogRead(UB_ANALOG_INPUT_PIN) > UBAT_MIN)
  {
    ...
    // Condensation phase
    LEDdisplay.showAnimation(CAL_1, FRAMES(CAL_1), TIME_MS(500)); // Total delay = 1 second
  }

  // Second calibration voltage. 18.0 AFR
  AFR = 18.0;
  UpdateAnalogOutput();
  ...
  while (UHeater < 13.0 && analogRead(UB_ANALOG_INPUT_PIN) > UBAT_MIN)
  {
    ...
    // Heat-up phase
    LEDdisplay.showAnimation(CAL_2, FRAMES(CAL_2), TIME_MS(500)); // Total delay = 1 second
  }
...
void loop()
{
  ...
  if (CJ125_Status == CJ125_DIAG_REG_STATUS_OK)
  {
    ...
    AFR = (LAMBDA_VALUE * STOICH_PETROL);
